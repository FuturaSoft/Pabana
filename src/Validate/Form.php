...
class Form
{
    ...
    public static function validate($aValidRule, $aData)
    {
        ...
        foreach ($aValidRule as $sField => $aFieldRule) {
            $bOptional = false;
            if (isset($aFieldRule['optionnal'])) {
                $bOptional = $aFieldRule['optionnal'];
            }
            if (isset($aFieldRule['optional'])) {
                $bOptional = $aFieldRule['optional'];
            }
            if ($bOptional === true && $aData[$sField] == '') {
                continue;
            }
            // Parcours les type de validation
            foreach ($aFieldRule as $sRuleType => $aRuleData) {
                if ($sRuleType !== 'prepare') {
                    $aReturn = array(
                        'name' => $sField,
